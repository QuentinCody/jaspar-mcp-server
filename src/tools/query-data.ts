...
interface QueryEnv {
    JASPAR_DATA_DO?: unknown;
}

export function registerQueryData(server: McpServer, env?: QueryEnv): void {
    const handler = createQueryDataHandler("JASPAR_DATA_DO", "jaspar");

    server.registerTool(
        "jaspar_query_data",
        {
            title: "Query Staged JASPAR Data",
            description:
                "Query staged JASPAR data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
                data_access_id: z
                    .string()
                    .min(1)
